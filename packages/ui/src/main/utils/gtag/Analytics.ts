...
const FETCH_TIMEOUT_IN_MS = 3000
...
class Analytics {
  ...
  isEnabled() {
    return Boolean(MEASUREMENT_ID && API_SECRET && MEASUREMENT_ID !== '<measurement_id>' && API_SECRET !== '<api_secret>')
  }
  ...
  async fireEvent(
    ...
    try {
      ...
      if (this.debug) {
        console.warn('Google Analytics request failed with an exception', e)
      }
    } finally {
